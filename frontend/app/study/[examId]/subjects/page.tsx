...
function SubjectCard({ subject, examId }: { subject: any, examId: string }) {
  const theme = subjectThemes[subject.title.toUpperCase()] || defaultTheme;
  const Icon = theme.icon;
  
  return (
    <Link href={`/study/${examId}/subjects/${subject.id}`}>
      <motion.div 
...
export default function StudyLandingPage() {
  ...
  const selectedExamId = examIdFromUrl || profile?.exam || "Group_II";
